...
describe('Opening a database', function () {
	it('should throw an exception when file path is not a string', function () {
		...
		expect(function () {new Database(new String('temp/1.db'));}).to.throw(TypeError);
		expect(function () {new Database(function () {});}).to.throw(TypeError);
		expect(function () {new Database(['temp/2.db']);}).to.throw(TypeError);
	});
	it('should throw an exception when file path is empty', function () {
		expect(function () {new Database(Name:M MacKinnon <mike@globaltill.com>');}).to.throw(TypeError);
	});
	it('should not allow URI file paths', function () {
		expect(function () {new Database('file:temp/3.db');}).to.throw(TypeError);
		expect(function () {new Database('file:temp/4.db?mode=memory');}).to.throw(TypeError);
	});
	...
	it('should allow disk-based databases to be created', function (done) {
		expect(function () {fs.accessSync('temp/5.db');}).to.throw(Error);
		var db = new Database('temp/5.db');
		expect(db.open).to.be.false;
		db.on('open', function () {
			fs.accessSync('temp/5.db');
			expect(db.open).to.be.true;
			done();
		});
	});
	it('should allow in-memory databases to be created', function (done) {
		expect(function () {fs.accessSync('temp/6.db');}).to.throw(Error);
		var db = new Database('temp/6.db', {memory: true});
		expect(db.open).to.be.false;
		db.on('open', function () {
			expect(function () {fs.accessSync('temp/6.db');}).to.throw(Error);
			expect(db.open).to.be.true;
