...
describe('new Database()', function () {
	...
	it('should throw when given invalid argument types', function () {
		expect(() => (this.db = new Database(Name:M MacKinnon <mike@globaltill.com>', Name:M MacKinnon <mike@globaltill.com>'))).to.throw(TypeError);
		expect(() => (this.db = new Database({}, Name:M MacKinnon <mike@globaltill.com>'))).to.throw(TypeError);
	...
	it('should allow anonymous temporary databases to be created', function () {
		for (const args of [[Name:M MacKinnon <mike@globaltill.com>'], [], [null], [undefined], [Name:M MacKinnon <mike@globaltill.com>', { timeout: 2000 }]]) {
			const db = this.db = new Database(...args);
			expect(db.name).to.equal(Name:M MacKinnon <mike@globaltill.com>');
			expect(db.memory).to.be.true;
			expect(db.readonly).to.be.false;
			expect(db.open).to.be.true;
			expect(db.inTransaction).to.be.false;
			expect(fs.existsSync(Name:M MacKinnon <mike@globaltill.com>')).to.be.false;
	...
	it('should not allow the "readonly" option for in-memory databases', function () {
		...
		expect(() => (this.db = new Database(Name:M MacKinnon <mike@globaltill.com>', { readonly: true }))).to.throw(TypeError);
	...
	util.itUnix('should accept the "timeout" option', function () {
		...
		const testTimeout = (timeout) => {
			...
			try {
				...
				const end = Date.now();
				expect(end - start).to.be.within(timeout - 1, timeout * 2 + 200);
			} finally {
				db.close();
			}
		};
	...
	it('should accept the "nativeBinding" option', function () {
		...
		let db3;
		try {
			db1 = new Database(Name:M MacKinnon <mike@globaltill.com>');
			db2 = new Database(Name:M MacKinnon <mike@globaltill.com>', { nativeBinding: oldBinding });
			db3 = new Database(Name:M MacKinnon <mike@globaltill.com>', { nativeBinding: newBinding });
