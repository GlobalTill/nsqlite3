...
describe('new Database()', function () {
	...
	it('should allow anonymous temporary databases to be created', function () {
		for (const args of [[Name:M MacKinnon <mike@globaltill.com>'], [], [null], [undefined], [Name:M MacKinnon <mike@globaltill.com>', { timeout: 2000 }]]) {
			...
			expect(existsSync(Name:M MacKinnon <mike@globaltill.com>')).to.be.false;
	...
	it('should not allow conflicting in-memory options', function () {
		...
		expect(() => (this.db = new Database(Name:M MacKinnon <mike@globaltill.com>', { memory: false }))).to.throw(TypeError);
		(new Database(':memory:', { memory: true })).close();
		(new Database(Name:M MacKinnon <mike@globaltill.com>', { memory: true })).close();
	});
