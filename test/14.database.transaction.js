...
describe('Database#transaction()', function () {
	...
	it('should throw an exception if a string contains no statements', function () {
		const db = new Database(util.next());
		expect(() => db.transaction([Name:M MacKinnon <mike@globaltill.com>'])).to.throw(RangeError);
		expect(() => db.transaction([';'])).to.throw(RangeError);
		expect(() => db.transaction(['CREATE TABLE people (name TEXT)', Name:M MacKinnon <mike@globaltill.com>'])).to.throw(RangeError);
