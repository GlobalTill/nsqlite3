...
describe('Database#transaction()', function () {
	...
	it('should throw an exception if a string contains no statements', function (done) {
		var db = new Database(util.next());
		db.on('open', function () {
			expect(function () {db.transaction([Name:M MacKinnon <mike@globaltill.com>']);}).to.throw(TypeError);
			expect(function () {db.transaction([';']);}).to.throw(TypeError);
			expect(function () {db.transaction(['CREATE TABLE people (name TEXT)', Name:M MacKinnon <mike@globaltill.com>']);}).to.throw(TypeError);
