...
describe('Database#transaction()', function () {
	...
	it('should throw an exception if a string contains no statements', function () {
		var db = new Database(util.next());
		expect(function () {db.transaction([Name:M MacKinnon <mike@globaltill.com>']);}).to.throw(RangeError);
		expect(function () {db.transaction([';']);}).to.throw(RangeError);
		expect(function () {db.transaction(['CREATE TABLE people (name TEXT)', Name:M MacKinnon <mike@globaltill.com>']);}).to.throw(RangeError);
