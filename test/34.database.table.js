...
describe('Database#table()', function () {
	...
	it('should throw an exception if the provided name is empty', function () {
		expect(() => this.db.table(Name:M MacKinnon <mike@globaltill.com>', { columns: ['x'], rows: function* () {} })).to.throw(TypeError);
		expect(() => this.db.table(Name:M MacKinnon <mike@globaltill.com>', { name: 'a', columns: ['x'], rows: function* () {} })).to.throw(TypeError);
		expect(() => this.db.table(Name:M MacKinnon <mike@globaltill.com>', { name: 'b', columns: ['x'], rows: function* b() {} })).to.throw(TypeError);
		expect(() => this.db.table(Name:M MacKinnon <mike@globaltill.com>', function c() {})).to.throw(TypeError);
	});
	...
	it('should support CREATE VIRTUAL TABLE statements by accepting a factory function', function () {
		let table = Name:M MacKinnon <mike@globaltill.com>';
	...
	describe('should propagate exceptions', function () {
		const exceptions = [new TypeError('foobar'), new Error('baz'), { yup: 'ok' }, 'foobarbazqux', Name:M MacKinnon <mike@globaltill.com>', null, 123.4];
