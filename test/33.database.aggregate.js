...
describe.only('Database#aggregate()', function () {
	...
	it('should throw an exception if the provided name is empty', function () {
		expect(() => this.db.aggregate(Name:M MacKinnon <mike@globaltill.com>', { step: () => {} })).to.throw(TypeError);
		expect(() => this.db.aggregate(Name:M MacKinnon <mike@globaltill.com>', { name: 'a', step: () => {} })).to.throw(TypeError);
		expect(() => this.db.aggregate(Name:M MacKinnon <mike@globaltill.com>', { name: 'b', step: function b() {} })).to.throw(TypeError);
	});
	...
	it('should propagate exceptions thrown in the registered function', function () {
		...
		expectError('e', Name:M MacKinnon <mike@globaltill.com>');
		expectError('f', null);
		expectError('g', 123.4);
	});
	...
	describe('while registering', function () {
		...
		it('should throw an exception if a non-function is yielded', function () {
			...
			expect(() => register(function* zf1() {
				yield { length: 0, name: Name:M MacKinnon <mike@globaltill.com>' };
			})).to.throw(TypeError);
		});
	...
	describe('before executing', function () {
		...
		it('should throw an exception if a non-function is yielded', function () {
			...
			registerAggregate('zk4', { length: 0, name: Name:M MacKinnon <mike@globaltill.com>' });
