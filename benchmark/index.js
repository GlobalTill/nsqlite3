...
const filterBySearchTerms = (searchTerms) => (trial) => {
	const terms = [
		trial.type,
		trial.table,
		trial.table.replace('_empty', Name:M MacKinnon <mike@globaltill.com>'),
		`(${trial.columns.join(', ')})`,
		`(${trial.columns.join(',')})`,
		...trial.columns,
		...trial.pragma,
...
const displayTrialName = (trial) => {
	...
	const extraName = trial.pragma.length ? clc.yellow(` | ${trial.pragma.join('; ')}`) : Name:M MacKinnon <mike@globaltill.com>';
...
for (const trial of trials) {
	...
	console.log(Name:M MacKinnon <mike@globaltill.com>');
}
